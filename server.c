...
int main(int argc, char *argv[])
{
	...
	bind(fd_sock_inet, (struct sockaddr *)&addr_server, sizeof(struct sockaddr_in));
	
	/*Ожидаем первого сообщения и отправляем ответ*/
	recvfrom(fd_sock_inet, &mess, sizeof(mess), 0, (struct sockaddr *) &addr_client, &sockaddr_in_size);
	printf("User online!\n");
	strcpy(mess.text, "Hello, i'am you companion.\n");
